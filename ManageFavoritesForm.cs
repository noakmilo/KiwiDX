...
internal sealed class ManageFavoritesForm : Form
{
    ...
    internal ManageFavoritesForm(List<FavoriteServer> favorites, Func<List<FavoriteServer>, bool> save)
    {
        ...
        var notice = new Label { Dock = DockStyle.Fill, Text = "Only KiwiSDR network receivers or the Twente WebSDR exception may be added.\nUse the receiver's HTTP/HTTPS address. Other WebSDR servers are not supported." };
        ...
        buttons.Controls.AddRange(new Control[] { add, delete, close });
    ...
    private void AddFavorite()
    {
        ...
        var next = favorites.Append(new FavoriteServer(title.Text.Trim(), isTwente ? "[OFF Kiwi Network]" : description.Text.Trim(), normalized)).ToList();
        if (!save(next)) return;
        favorites = next;
        RefreshList();
